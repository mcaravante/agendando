...
const integrationInfo: Record<string, { name: string; descriptionEn: string; descriptionEs: string; icon: React.ReactNode }> = {
  ...
  ZOOM: {
    ...
    icon: (
      <svg className="w-8 h-8" viewBox="-573 -145 350 350">
        <rect x="-573" y="-145" width="350" height="350" rx="56" fill="#4A8CFF"/>
        <path fill="white" d="M-329.5,1.8l61.7-45.1c5.4-4.4,9.5-3.4,9.5,4.7V98.9c0,9.1-5.1,8.1-9.5,4.7l-61.7-45.1V1.8z M-538.3-38.2V64.2c0.1,23.2,19,41.9,42.2,41.8h149.5c4.2,0,7.6-3.4,7.7-7.6V-4.1c-0.1-23.2-19-41.9-42.2-41.8h-149.5C-534.8-45.8-538.2-42.5-538.3-38.2z"/>
      </svg>
    ),
  },
  MERCADOPAGO: {
    name: 'MercadoPago',
    descriptionEn: 'Accept payments for your events using MercadoPago Checkout Pro.',
    descriptionEs: 'Cobra por tus eventos usando MercadoPago Checkout Pro.',
    icon: (
      <svg className="h-14 w-auto" viewBox="0 0 1048.82 425.2">
...
export function Integrations() {
  ...
  const handleConnect = async (provider: string) => {
    ...
    try {
      ...
      if (provider === 'GOOGLE_CALENDAR') {
        endpoint = '/integrations/google/auth';
      } else if (provider === 'ZOOM') {
        endpoint = '/integrations/zoom/auth';
      } else {
  ...
  const confirmDisconnect = async () => {
    ...
    try {
      const endpoint = disconnectingProvider === 'GOOGLE_CALENDAR'
        ? '/integrations/google'
        : disconnectingProvider === 'MERCADOPAGO'
        ...
        {integrations.map((integration) => {
          ...
          return (
            <div
              ...
              className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow flex flex-col items-center text-center"
            >
              <div className="mb-4 h-14 flex items-center">{info.icon}</div>
